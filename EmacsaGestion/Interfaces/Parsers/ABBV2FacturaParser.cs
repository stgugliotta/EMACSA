...
{
    class ABBV2FacturaParser : FacturaOledbExcelParser
    {
        ...
        public override void applySpecificRules(FacturaDTO facturaDTO)
        {
            try
            {
                if ("RC".Equals(facturaDTO.idTipoComprobante))
                {
                ...
                else
                {
                    ...
                    else if ("RB".Equals(facturaDTO.idTipoComprobante))
                    {
                ...
                try
                {
                    facturaDTO.idDeudor = long.Parse(facturaDTO.idDeudor).ToString();
                }
                catch (Exception ex1)
                {
                }
