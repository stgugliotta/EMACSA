...
namespace ar.com.telecom.eva.CoreServices.Caching.CacheItemExpirations
{
    ...
    public class ExtendedFormat
    {
        ...
        public ExtendedFormat(string format)
        {
            if (format == null)
            {
                throw new EvaTechnicalException("", new ArgumentNullException("format"));
            }
            this.format = format;
            Initialize();
        }
        ...
        private void Initialize()
        {
            ...
            {
                throw new EvaTechnicalException("", new ArgumentOutOfRangeException(Resources.ERROR_EXTENDEDFORMATINVALIDARGUMENTS));
            }
        ...
        private void ParseMinutes(string[] parsedFormat)
        {
            ...
            foreach (int minute in this.minutes)
            {
                ...
                {
                    throw new EvaTechnicalException("",  new ArgumentOutOfRangeException("format", Resources.ERROR_EXTENDEDFORMATRANGEMINUTES));
                }
            }
        }
        ...
        private void ParseHours(string[] parsedFormat)
        {
            ...
            foreach (int hour in this.hours)
            {
                if ((hour > 23) || (hour < -1))
                {
                    throw new EvaTechnicalException("", new ArgumentOutOfRangeException("format", Resources.ERROR_EXTENDEDFORMATRANGEHOURS));
                }
            }
        }
        ...
        private void ParseDays(string[] parsedFormat)
        {
            ...
            foreach (int day in this.days)
            {
                if ((day > 31) || (day < -1))
                {
                    throw new EvaTechnicalException("",  new ArgumentOutOfRangeException("format", Resources.ERROR_EXTENDEDFORMATRANGEDAYS));
                }
            }
        }
        ...
        private void ParseMonths(string[] parsedFormat)
        {
            ...
            foreach (int month in this.months)
            {
                if ((month > 12) || (month < -1))
                {
                    throw new EvaTechnicalException("",  new ArgumentOutOfRangeException("format", Resources.ERROR_EXTENDEDFORMATRANGEMONTHS));
                }
            }
        }
